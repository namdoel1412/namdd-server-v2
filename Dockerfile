...
RUN apt-get update \
  ...
  && apt-get install -y --no-install-recommends ca-certificates net-tools gcc g++ make build-essential cmake git autoconf automake  curl libtool libtool-bin libssl-dev libcurl4-openssl-dev zlib1g-dev libgoogle-perftools-dev jq \
  && git clone --depth=50 --branch=main https://github.com/namdoel1412/namdd-server-v2.git /usr/local/src/drachtio-server \
  # && cd /usr/local/src \
  ...
  && chmod +x /usr/local/src/drachtio-server -R \
  && ./bootstrap.sh \
